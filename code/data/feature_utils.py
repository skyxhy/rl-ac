...
class GraphFeatureBuilder:
    ...
    def _get(self, key, compute_fn):
        """惰性计算：存在则直接返回，否则计算并缓存"""
        if key not in self._cache:
            self._cache[key] = compute_fn()
        return self._cache[key]

    def _calc_grouped(self): return self._df.groupby("sender_pseudo")
    def _calc_node_ids(self): return sorted(self._grouped.groups.keys())
    ...
    def _calc_recv_sets(self):
        recv_sets = {}
        for nid in self._get("node_ids", self._calc_node_ids):
            recv_sets[nid] = set(self._grouped.get_group(nid)["receiver_id"].tolist())
        return recv_sets

    def _calc_edges(self):
        pairs, weights = [], []
        recv_sets = self._get("recv_sets", self._calc_recv_sets)
        node_ids = self._get("node_ids", self._calc_node_ids)
    ...
    def _calc_degrees(self):
        ...
        deg = np.zeros(len(self._get("node_ids", self._calc_node_ids)), dtype=float)
    ...
    def _calc_recv_entropy(self, p, g):
        ...
        c = Counter(items)
        total = sum(c.values())
        p_arr = np.array(list(c.values()), dtype=np.float64) / max(total, 1)
    ...
    def _calc_gap_cv(self, p, g):
        m, s = self._calc_gap_mean(p, g), self._calc_gap_std(p, g)
    ...
    def _calc_deg(self, p, g):
        ...
        return float(deg[self._get("node_ids", self._calc_node_ids).index(p)])

    def _calc_wdeg(self, p, g):
        _, wdeg = self._get("degrees", self._calc_degrees)
        return float(wdeg[self._get("node_ids", self._calc_node_ids).index(p)])

    def build(self, window_df: pd.DataFrame, window_idx: int = None):
        if window_df.empty: return None
        self._cache = {}
        self._df = window_df
        self._grouped = self._calc_grouped()
        self._node_ids = self._get("node_ids", self._calc_node_ids)
        if len(self._node_ids) < 2: return None

        node_map = {nid: i for i, nid in enumerate(self._node_ids)}
        feats, labels, rows = [], [], []

        for pseudo in self._node_ids:
            g = self._grouped.get_group(pseudo)
            feat_vec, row = [], {"window_idx": window_idx, "sender_pseudo": pseudo, "label": int(g["attack_type"].max())}
        ...
        edge_attr = torch.tensor(ew, dtype=torch.float).view(-1, 1) if ew.ndim == 1 else torch.tensor(ew, dtype=torch.float)

        # ✅ 关键修改：Data 对象只包含标准张量，不包含 node_ids/label_map 等复杂结构
        data = Data(
        ...
        # ✅ 关键修改：元数据单独打包返回，不塞进 Data 对象
        snapshot_dict = {
            "window_idx": window_idx,
            "node_ids": self._node_ids,
            "node_map": node_map,
            "feat_map": {nid: torch.tensor(f, dtype=torch.float) for nid, f in zip(self._node_ids, feats)},
            "label_map": {nid: int(self._grouped.get_group(nid)["attack_type"].max()) for nid in self._node_ids},
            "edge_index": edge_index,
...
def build_sequence_sample(seq_snapshots: list, feature_names: list) -> dict | None:
    """严格对齐原逻辑的序列样本构造器"""
    if not seq_snapshots: return None
    
    # 1. 统一节点索引（与原逻辑完全一致）
    node_id_set = set()
    for snap in seq_snapshots:
        node_id_set.update(snap["node_ids"])
    node_ids = sorted(list(node_id_set))
    ...
    # 2. 填充特征与边（确保 contiguous 且类型安全）
    for t, snap in enumerate(seq_snapshots):
        for nid in snap["node_ids"]:
            idx_local = node_map[nid]
            # 显式转换并保证内存连续，避免 ST-GCN 底层 stride 报错
            x_seq[t, idx_local] = torch.as_tensor(snap["feat_map"][nid], dtype=torch.float).contiguous()
            active_mask_seq[t, idx_local] = True
